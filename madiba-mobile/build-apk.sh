#!/usr/bin/env bash
# Build the MADIBA signed .apk. RUN ON A MACHINE WITH: Android Studio (or cmdline-tools),
# JDK 17+, Node 18+. App ID: za.gods.madiba
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

echo "==> 1/5 Build the web app (madiba-app)"
( cd "../madiba-app" && npm install && npm run build )
rm -rf www && mkdir www && cp -r "../madiba-app/dist/"* www/
