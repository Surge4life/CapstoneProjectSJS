#!/usr/bin/env bash
# Build the TS Industries signed .apk. RUN ON A MACHINE WITH: Android Studio (or cmdline-tools),
# JDK 17+, Node 18+. App ID: za.gods.ts
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

echo "==> 1/5 Build the web app (ts-app)"
( cd "../ts-app" && npm install && npm run build )
rm -rf www && mkdir www && cp -r "../ts-app/dist/"* www/
