#!/usr/bin/env bash
# Build the GODS Portals signed .apk. RUN ON A MACHINE WITH: Android Studio (or cmdline-tools),
# JDK 17+, Node 18+. App ID: za.gods.portals
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

echo "==> 1/5 Build the web app (portals-web)"
( cd "../portals-web" && npm install && npm run build )
rm -rf www && mkdir www && cp -r "../portals-web/dist/"* www/
