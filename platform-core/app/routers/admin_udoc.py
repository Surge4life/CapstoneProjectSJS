...
def demo_ready(db: Session = Depends(get_db), user: dict = Depends(principal)):
    """Capstone: auto-heal model-001 + demo pack to ACTIVE if suspended (smoke path)."""
    ...
    healed = []
    if model and (model.status or "").upper() != "ACTIVE":
        prev = model.status
        model.status = "ACTIVE"
        healed.append(f"model-001 {prev}->ACTIVE")
    if pack and (pack.status or "").upper() != "ACTIVE":
        prev = pack.status
        pack.status = "ACTIVE"
