...
def _ensure_division_staff_seed():
    """Internal GODS division staff — operator role, password staff123 (Capstone demo)."""
    from sqlalchemy import select
    from app.db.session import SessionLocal
    from app.db.models import User
    from app.core.security import hash_password
    staff = [
        ("seths@gods.local", "SETHS"),
        ("madiba@gods.local", "MADIBA"),
        ("ts@gods.local", "TS"),
    ]
    pw = os.environ.get("GODS_STAFF_PASSWORD", "staff123")
    db = SessionLocal()
    try:
        for email, division in staff:
            u = db.execute(select(User).where(User.email == email)).scalar_one_or_none()
            if not u:
                db.add(User(email=email, password_hash=hash_password(pw), role="operator",
                            division=division, active=True))
                print(f"[seed] division staff {email} · {division}")
            else:
                if (u.role or "") != "operator" or (u.division or "") != division:
                    u.role = "operator"; u.division = division; u.active = True
        db.commit()
    finally:
        db.close()
