# MADIBA — Desktop App (.exe via Electron)

Wraps the live MADIBA site in a native desktop window. It loads your deployed
...
```
Output in `release/`:
- `MADIBA Setup 1.0.0.exe`  — installer (creates Start-menu + desktop shortcut)
- `MADIBA 1.0.0.exe`        — portable (run directly, no install)
