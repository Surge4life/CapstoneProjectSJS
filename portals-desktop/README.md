# GODS Portals — Desktop App (.exe via Electron)

Wraps the live GODS Portals site in a native desktop window. It loads your deployed
...
```
Output in `release/`:
- `GODS Portals Setup 1.0.0.exe`  — installer (creates Start-menu + desktop shortcut)
- `GODS Portals 1.0.0.exe`        — portable (run directly, no install)
