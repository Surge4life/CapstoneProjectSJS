#!/usr/bin/env bash
# Build the SETHS signed .apk. RUN ON A MACHINE WITH: Android Studio (or cmdline-tools),
# JDK 17+, Node 18+. App ID: za.gods.seths
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

echo "==> 1/5 Build the web app (seths-app)"
( cd "../seths-app" && npm install && npm run build )
rm -rf www && mkdir www && cp -r "../seths-app/dist/"* www/
